...
class SKConv(nn.Module):
    def __init__(self, features, input_resolution, WH, M, G, r, stride=1, L=32):
        """ Constructor
        Args:
    ...
    def forward(self, x):
        ...
        fea_v = fea_v.flatten(2).transpose(1, 2)
        fea_v = self.norm(fea_v)
        return fea_v
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, SK=False):
        super().__init__()
        self.dim = dim
        self.SK = SK
        self.input_resolution = input_resolution
        ...
        # if self.SK:
        #     self.skconv = SKConv1(self.dim, self.input_resolution, WH=1, M=3, G=1, r=2)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        shape = (self.input_resolution[0] // 2, self.input_resolution[1] // 2)
        if 'use_SK' in os.environ and os.environ['use_SK'] == 'True':
            self.SK = True
        else:
            self.SK = False
        ...
        if self.SK:
            # self.skconv = SKConv(self.dim*2, shape, WH=1, M=3, G=1, r=2)
            from .layers.selective_kernel import SelectiveKernel
            self.skconv = SelectiveKernel(self.dim*2).to(next(self.reduction.parameters()).device)
    ...
    def forward(self, x):
        ...
        if self.SK:
            x = x.transpose(1,2).view(B,2*C,H//2,W//2) # b h*w c -> b c h w
            x = self.skconv(x) # b c h w
            x = x.view(B,2*C,-1).transpose(1,2) # b h*w c
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, SK=False):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for index, i_layer in enumerate(range(self.num_layers)):
            layers += [BasicLayer(
                ...
                # SK=False)]
                SK=True if index in [0,1,2,3] else False)]
